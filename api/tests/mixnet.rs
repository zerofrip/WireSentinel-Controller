...
async fn spawn_test_server() -> (String, tokio::task::JoinHandle<()>) {
    ...
    let policy = ControllerSecurityPolicy {
        jwt_secret: "mixnet-test-secret".into(),
        bcrypt_cost: 4,
        ..Default::default()
    };
...
#[tokio::test]
async fn integration_mixnet_heartbeat_and_list() {
    ...
    let heartbeat = client
        .post(format!("{base}/api/v1/agents/{device_id}/mixnet/heartbeat"))
        .json(&json!({
    ...
    let inventory = client
        .get(format!("{base}/api/v1/mixnet"))
        .header("Authorization", format!("Bearer {token}"))
        .send()
        .await
        .expect("inventory");
    assert!(inventory.status().is_success());
    let inv: Value = inventory.json().await.expect("json");
    assert_eq!(inv["node_count"], 1);
    assert_eq!(inv["route_count"], 1);

    let routes = client
        .get(format!("{base}/api/v1/mixnet/routes"))
        .header("Authorization", format!("Bearer {token}"))
        .send()
        .await
        .expect("routes");
    ...
    assert_eq!(route_list[0]["label"], "default");

    let health = client
        .get(format!("{base}/api/v1/mixnet/health"))
        .header("Authorization", format!("Bearer {token}"))
        .send()
        .await
        .expect("health");
...
#[tokio::test]
async fn integration_mixnet_heartbeat_requires_device() {
    let (base, handle) = spawn_test_server().await;
    let client = reqwest::Client::new();
    let resp = client
        .post(format!("{base}/api/v1/agents/missing-device/mixnet/heartbeat"))
        .json(&json!({"mixnet_connected": false, "stub_mode": true, "active_routes": []}))
        .send()
        .await
        .expect("heartbeat");
